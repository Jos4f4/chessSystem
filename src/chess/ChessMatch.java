...
public class ChessMatch {
	...
	public ChessPiece replacePromotedPice(String type) {
		if(promoted == null) {
			throw new IllegalStateException("There is no peace do be promoted");
		}
	...
	private void validateSourcePosition(Position position) {
		...
		if(!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("There is not possible movesfor the chosen piece.");
		}
	}
			...
			for (int i=0; j<board.getRows(); i++) {
				for(int j=0; j<board.getColumns(); j++) {
					if(mat[i][j]) {
						...
						Piece capturedPiece = makeMOve(source, target);
						boolean testCheck = testCheck(source);
						undoMove(source, target, capturedPiece);
						if(!testCheck) {
							return false;
						}
